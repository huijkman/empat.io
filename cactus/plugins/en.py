...
Global = {"config": {}, "posts-en": []}

Global["config"]["path"] = "people"
...
def preBuild(site):
	...
	for page in site.pages():
		...
		if page.path.startswith("%s/" % Global["config"]["path"]):
			...
			Global["posts-en"].append(context_post)

	# Sort the posts by date and add the next and previous page indexes
	Global["posts-en"] = sorted(Global["posts-en"], key=lambda x: x.get("date"))
	Global["posts-en"].reverse()
	
	indexes = xrange(0, len(Global["posts-en"]))
	
	for i in indexes:
		if i+1 in indexes: Global["posts-en"][i]['prevPost'] = Global["posts-en"][i+1]
		else: Global["posts-en"][i]['prevPost'] = Global["posts-en"][0]
		if i-1 in indexes: Global["posts-en"][i]['nextPost'] = Global["posts-en"][i-1]
		else: Global["posts-en"][i]['nextPost'] = Global["posts-en"][len(Global["posts-en"])-1]


def preBuildPage(site, page, context, data):
	
	context['postsEN'] = Global["posts-en"]
	
	for post in Global["posts-en"]:
